...
function Do-Default-Build-Service {
  New-Item -ItemType directory -Path "${pkg_prefix}/hooks"

Add-Content -Path "$pkg_prefix/hooks/run" -Value @"
function Chef-Client {
  {{pkgPathFor "thelunaticscripter/chef-client"}}/chef/bin/chef-client.bat -z -l {{cfg.log_level}} -c $pkg_svc_config_path/client-config.rb -j $pkg_svc_config_path/attributes.json --once --no-fork --run-lock-timeout {{cfg.run_lock_timeout}}
}
...
function Do-Default-Build {
  if(!Test-Path -Path "${scaffold_policyfiles_path}"){
...
"@
  ...
  if(!Test-Path -Path "${scaffold_data_bags_path}"){
