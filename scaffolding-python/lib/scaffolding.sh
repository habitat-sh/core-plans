...
pkg_scaffolding_python_runtime_pkg_name="core/python"
# Default venv command to use.
pkg_scaffolding_python_venv_cmd="python -m venv"
...
scaffolding_python_venv() {
  if [[ ! -f "$pkg_prefix/bin/activate" ]]; then
    build_line "Creating package venv"
    $pkg_scaffolding_python_venv_cmd \
      --symlinks \
      "$pkg_prefix"
  fi

  if source "$pkg_prefix/bin/activate"; then
    build_line "Activated venv"
  fi
}
