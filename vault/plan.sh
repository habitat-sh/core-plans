...
do_install() {
  ...
  chown -R "$pkg_svc_user":"$pkg_svc_group" "${pkg_prefix}"
  chown "$pkg_svc_user":"$pkg_svc_group" "${pkg_prefix}"/bin/vault

  chown "$pkg_svc_user":"$pkg_svc_group" "${pkg_svc_config_path}"/settings.hcl
