title 'Tests to confirm erlang22 works as expected'

plan_origin = ENV['HAB_ORIGIN']
plan_name = input('plan_name', value: 'erlang22')

control 'core-plans-erlang22-works' do
  impact 1.0
  title 'Ensure erlang22 works as expected'
  desc '
  Verify erlang22 by ensuring that
  ...
  plan_pkg_version = plan_installation_directory.stdout.split("/")[5]
  {
    ...
    if(script)
      Tempfile.open('foo') do |f|
        f << script
        sleep(1) # ensure file exists first
